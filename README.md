...
- 隐形访问统计（不蒜子）：主页只加载计数脚本、不显示任何数字（注意：不蒜子会强制显示带 `busuanzi_container_*` id 的元素，即使加了 hidden，所以前台不能放这类节点），访问量在 `/stats.html` 查看（站内无链接、noindex）
- 简报推送：每次有新论文入库时，GLM 额外生成一段「今日速览」写入 `data/*.json` 并展示在页面顶部，同时经 Server酱 推送到微信（Secret `SERVERCHAN_SENDKEY`，未配置则静默跳过；免费额度 5 条/天足够）。每天 17:00 UTC 主跑若一无所获会推一条「今日无新增」心跳，补跑/手动触发不重复推送；主跑新增不足 `auto_classics_below`（默认 5）篇时自动追加一轮经典库更新，保证每天有内容可读
