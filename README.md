...
- **候选发现与筛选零 LLM 成本**：Semantic Scholar 检索各技术线近五年论文，纯按引用数门槛过滤（当年与去年 ≥40，更早的每年递增 40，即 40 × max(1, 距今年数)）；
- **每篇经典只评审一次**：结果写入 `data/classics.json` 永久复用；已在每日库中的论文升级为经典时直接复用现有中文研判（零新增调用）；
- **每次运行最多评审 `classic.max_new_per_run` 篇**（默认 8，按引用数从高到低入场），首轮积压会分摊到后续运行；想加快建设速度，手动多触发几次该 workflow 或临时调大该值；
